...
export default function MessengerComponent() {
      ...
      const fetchCurrentConversations = async (ConversationId) => {
        try {
          const res = await axios.get(
            `http://localhost:8800/api/conversations?id=${ConversationId}`
          );
  ...
  useEffect(() => {
    ...
    const getFriends = async () => {
      const res = await axios.get(
        "http://localhost:8800/api/relationships/friendsdata?userId=" +
          currentUser.id
      );
  ...
  useEffect(() => {
    socket.current.on("getUsers", (users) => {
      const onlineVariable = friends.filter((f) =>
        users.some((u) => u.userId === f.id)
      );

      //update online users
      setOnlineUsers(onlineVariable);
    });
  }, [friends, currentUser]);
  ...
  const getUserFromConversation = async (conversation) => {
    try {
      ...
      const res = await axios(
        "http://localhost:8800/api/users/find/" + friendId
      );
                ...
                <div className="header-chat">
                  ...
                  <div className="userInfo">
                    {currentChatUser.profilePic && (
                      <img
                        src={`/upload/${currentChatUser.profilePic}`}
                        alt={currentChatUser.username}
                      />
                    )}
